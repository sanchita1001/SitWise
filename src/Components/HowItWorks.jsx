...
const steps = [
  {
    ...
    description: "Sign in securely with your Thapar University Google account to access the booking system.",
  },
  {
    icon: <BookOpen className="w-8 h-8 sm:w-10 sm:h-10" />,
    title: "Choose Your Floor",
    description: "Browse real-time seat availability on any floor and pick your preferred study environment.",
  },
  {
    icon: <CalendarCheck className="w-8 h-8 sm:w-10 sm:h-10" />,
    title: "Book Instantly",
    description: "Reserve your seat in just a click. Your spot is held for 5 minutes until you check in.",
  },
  {
    icon: <Timer className="w-8 h-8 sm:w-10 sm:h-10" />,
    title: "Quick Check-In",
    description: "Arrive at the library and confirm your seat within 5 minutes to secure your booking.",
  },
  {
    icon: <CheckCircle className="w-8 h-8 sm:w-10 sm:h-10" />,
    title: "Enjoy Your Study Time",
    description: "Relax and focus—your seat is guaranteed. Free your seat when done for others to use.",
  },
];
      ...
      <div className="flex flex-col gap-6 sm:gap-8 md:flex-row md:gap-6 lg:gap-8 justify-center items-center max-w-5xl mx-auto">
        {steps.map((step, i) => (
          ...
          >
            ...
            <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 text-gray-900">{step.title}</h3>
            <p className="text-gray-500 text-xs sm:text-sm md:text-base">{step.description}</p>
            <motion.div
